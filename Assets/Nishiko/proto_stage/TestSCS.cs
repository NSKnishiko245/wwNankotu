...
public class TestSCS : MonoBehaviour
{
    ...
    private IEnumerator TurnOnScreenShotEx()
    {
        ...
        //ここにおる方向に対してのサイズを決める
        Vector2Int size = new Vector2Int(Screen.width/2, Screen.height);
        Texture2D tex = new Texture2D(size.x, size.y, TextureFormat.ARGB32, false);

        // 引数で指定した矩形内をスクショし、そのデータを自身(tex)に格納
        float addx=0;
        tex.ReadPixels(new Rect(0.0f+addx, 0.0f, size.x+addx, size.y), 0, 0);
        tex.Apply();
